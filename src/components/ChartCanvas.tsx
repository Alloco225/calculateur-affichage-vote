...
export const ChartCanvas = ({ title, label, labelValue, candidates }: ChartCanvasProps) => {
    ...
    <div className="relative w-full h-full flex items-start justify-center p-4 pt-8 sm:p-8 sm:pt-12" id="chart-canvas">
      <div className="w-full max-w-full sm:max-w-3xl lg:max-w-5xl">
        <div className="text-center mb-8">
          ...
          {label &&  <p className="text-lg sm:text-xl md:text-2xl text-white" >
              {label}
              
              {labelValue && <div className="text-3xl sm:text-4xl md:text-5xl font-bold uppercase text-white">
                {labelValue}
              </div>}
            </p>}        
        </div>
        ...
        {sortedCandidates.length === 0 ? (
          ...
          <div className="flex items-end justify-between gap-2 sm:gap-4 h-[300px] sm:h-[400px] md:h-[500px] border-b-4 pb-3 border-[#B30A0C]">
            {sortedCandidates.map((candidate) => {
              ...
              return (
								<div key={candidate.id} className="relative h-full flex flex-col items-center justify-end flex-1 max-w-[120px] sm:max-w-[160px] md:max-w-[200px]">
									<div
                    ...
                    <div className="text-center pt-2">
                      <span className="text-white font-bold text-xl sm:text-2xl md:text-3xl drop-shadow-lg">{percentage.toFixed(2)}%</span>
                    </div>
										<div
											className=""
											style={{
												height: `${(percentage / 100) * 500}px`,
											}}>
										</div>

										{candidate.image && (
											<div className="h-[120px] sm:h-[160px] md:h-[200px] overflow-hidden flex justify-center">
												<img src={candidate.image} alt={candidate.name} className="max-h-full w-full object-cover" />
											</div>
										)}

										<div className="absolute bottom-0 left-0 w-full h-1/2 bg-gradient-to-b from-transparent to-black opacity-50"></div>
